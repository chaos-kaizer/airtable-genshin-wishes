...
let Banner = function(id,name,pity5Rate){
    ...
    this.outputBanner = async function(){
        ...
        for (let index in filteredRecords) {
            ...
            if(filteredRecords[i].getCellValue("Rank") == 4){
                this.fourStarAmout ++;
                this.last4Star = i+1;
            }
            else if(filteredRecords[i].getCellValue("Rank") == 5){
                this.fiveStarAmount ++;
                this.last5Star = i+1;
            }
        }
        ...
        //Output
        output.markdown('## '+this.bannerName+' ('+this.totalWish+' wishes)');
        output.markdown('**4**⭐ acquired **'+readableLast4Star+'** wish(es) ago. Pity reached in ** '+((this.last4Star+10)-this.totalWish)+'** wishes maximum.');
        output.markdown('**5**🌟 acquired **'+readableLast5Star+'** wish(es) ago. Pity reached in ** '+((this.last5Star+pity5Rate)-this.totalWish)+'** wishes maximum.');
        output.markdown('');
    }
}
